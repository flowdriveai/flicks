			...
			<div class="justify-between flex flex-col w-1/5 divide-y-2 divide-white/20">
				<div class="flex min-w-full items-center">
					<div class="p-5">
						<div class="pb-5">
							...
							<div>
								{dateConvert(driveData.started_on).toLocaleDateString('en-us', {
									weekday: 'long',
									month: 'short',
									day: 'numeric',
									hour: 'numeric',
				...
				<div class="flex flex-col mt-auto min-w-full divide-y-2 divide-white/20">
					...
					{#if driveData.owned}
						{#if driveData.shared}
							<button
								class="text-red-500  hover:text-white hover:bg-red-600 duration-300 text-center text-sm font-normal grow py-5 px-10"
								on:click|preventDefault={toggleShare}>Unshare Drive</button
							>
						{:else}
							<button
								class="text-green-500 hover:text-white hover:bg-green-600 duration-300 text-center text-sm font-normal grow py-5 px-10"
								on:click|preventDefault={toggleShare}>Share Drive</button
							>
						{/if}
					{:else}
						<button disabled class="text-white/60 text-center text-sm font-normal grow py-5 px-10"
							>Shared</button
						>
					{/if}
				</div>
